...
class TimetableApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Weekly Timetable with Excel Save")
        ...
        # Submit button to save data to Excel
        tk.Button(input_frame, text="Submit & Save to Excel", font=("Helvetica", 12, 'bold'),
                  bg='#27AE60', fg='white', command=self.submit_to_excel).grid(row=0, column=7, padx=10)
    ...
    def submit_to_excel(self):
        ...
        for row_idx, time in enumerate(TIMES, start=2):
            time_cell = ws.cell(row=row_idx, column=1, value=time)
            time_cell.font = Font(bold=True)
            time_cell.alignment = Alignment(horizontal="center")

            for col_idx, day in enumerate(DAYS, start=2):
                subject = self.entries[(day, time)].cget("text")
                ws.cell(row=row_idx, column=col_idx, value=subject if subject else "")
        ...
        wb.save(filename)
        messagebox.showinfo("Saved", f"Timetable saved to Excel file:\n{filename}")
